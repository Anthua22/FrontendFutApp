...
import { Categoria, Equipo, Partido, PartidoAdd, User } from 'src/app/models/models';
...
export class PartidoFormPage implements OnInit, AfterViewInit {
  ...
  constructor(private router: Router, private partidoService: PartidosService, private nav: NavController, private route: ActivatedRoute,
    private equipoService: EquipoService, private userService: UsersService, private toast: ToastController) { }
  ...
  obtnerCategoria() {
    this.equiposLocales = this.equiposLocales ? this.equiposLocales.filter(x => x.categoria === this.partido.categoria) : []
    this.equiposVisitantes = this.equiposVisitantes ? this.equiposVisitantes.filter(x => x.categoria === this.partido.categoria) : [];
    this.categoriaDisabled = false;
    switch (this.partido.categoria) {
      case Categoria.PRIMERA:
  ...
  arbitroElegido() {
    this.arbitrosSecundario = this.arbitrosSecundario ? this.arbitrosSecundario.filter(x => x._id !== this.partido.arbitro_principal._id) : [];
    this.arbitroSecundarioDisabled = false;
  }

  habilitarVisitantes() {
    this.partido.equipo_local = this.equiposLocales.find(x => x._id === this.idLocal);
    this.equiposVisitantes = this.equiposVisitantes ? this.equiposVisitantes.filter(x => x._id !== this.partido.equipo_local._id) : [];
    this.equipoVisitanteDisabled = false;
    this.partido.equipo_local = this.equiposLocales.find(x => x._id === this.partido.equipo_local._id);
